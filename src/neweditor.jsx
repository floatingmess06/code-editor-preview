...
import { Box, Paper, AppBar, Toolbar, Typography, Tab, Tabs, Button, IconButton, Snackbar, Alert, Drawer, List, ListItem, ListItemIcon, ListItemText, Divider, Menu, MenuItem, Tooltip, Zoom } from '@mui/material';
...
const PreviewContainer = styled(Box)(({ theme, deviceFrame }) => {
  ...
  return {
    ...
    margin: '0 auto',
    overflow: 'hidden',
    borderRadius: deviceFrame === 'none' ? '6px' : frames[deviceFrame].borderRadius,
    border: deviceFrame === 'none' ? 'none' : '10px solid #222',
    boxShadow: deviceFrame === 'none' ? 'none' : '0 25px 50px -12px rgba(0, 0, 0, 0.25)',
    background: '#000',
    transition: 'all 0.3s ease',
  };
});
...
const CodeEditor = () => {
  ...
  const TabContent = ({ index, value, children }) => {
    return (
      ...
      >
        {value === index && (
          <Zoom in={value === index} style={{ transitionDelay: value === index ? '100ms' : '0ms' }}>
    ...
    <Box sx={{ 
      ...
      <GlassAppBar position="static" color="default">
        <Toolbar>
          ...
          >
            <MenuItem onClick={() => handleSetLayout('split')}>Split View</MenuItem>
            <MenuItem onClick={() => handleSetLayout('code-focus')}>Code Focus</MenuItem>
            <MenuItem onClick={() => handleSetLayout('preview-focus')}>Preview Focus</MenuItem>
            <Divider />
            <MenuItem onClick={() => handleDeviceFrameChange('none')}>
              <ListItemIcon><TvIcon fontSize="small" /></ListItemIcon>
              <ListItemText>No Frame</ListItemText>
            </MenuItem>
            <MenuItem onClick={() => handleDeviceFrameChange('mobile')}>
              <ListItemIcon><SmartphoneIcon fontSize="small" /></ListItemIcon>
              <ListItemText>Mobile Frame</ListItemText>
            </MenuItem>
            <MenuItem onClick={() => handleDeviceFrameChange('tablet')}>
              <ListItemIcon><TabletIcon fontSize="small" /></ListItemIcon>
            ...
            <Box sx={{ display: 'flex', px: 1 }}>
              ...
              <Tooltip title={editorExpanded ? "Exit fullscreen" : "Fullscreen"}>
                <IconButton size="small" onClick={handleToggleEditorExpanded}>
                  {editorExpanded ? <FullscreenExitIcon fontSize="small" /> : <FullscreenIcon fontSize="small" />}
                </IconButton>
              </Tooltip>
            </Box>
          </Box>
          ...
          <Box sx={{ 
            ...
            <Box sx={{ display: 'flex' }}>
              <Tooltip title="Download project">
                <IconButton size="small" onClick={handleDownloadProject} sx={{ mr: 1 }}>
          ...
          <List disablePadding>
            <ListItem disablePadding sx={{ mb: 1 }}>
              <ListItemButton onClick={handleToggleDarkMode}>
                <ListItemIcon>
                  {darkMode ? <LightModeIcon color="primary" /> : <DarkModeIcon color="primary" />}
                </ListItemIcon>
